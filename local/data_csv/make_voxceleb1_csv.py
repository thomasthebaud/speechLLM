...
for set_ in ['dev', 'test']:
    labels = {'audio_path':[], 'audio_len':[], 'gender':[], 'accent':[], 'speaker':[]}
    ...
    print(df.shape)
    save_csv(df, 'voxceleb1', set_)

    if set_=='dev':
        df_train, df_dev = train_test_split(df, test_size=0.1)
        save_csv(df_train, 'voxceleb1', set_+'_train')
        save_csv(df_dev, 'voxceleb1', set_+'_valid')
